...
const char HTTP_SCRIPT_SMARTMETER_JS[] PROGMEM =
    "<script>"
    "var meterData=[];"
    "var selectedMeter=null;"
    "function loadSmartMeterList(){"
    "  var xhr=new XMLHttpRequest();"
    "  xhr.open('GET','https://raw.githubusercontent.com/iot-maker/tasmota-smart-meter-scripts/main/api/list.json',true);"
    "  xhr.onreadystatechange=function(){"
    "    if(xhr.readyState==4&&xhr.status==200){"
    "      try{"
    "        meterData=JSON.parse(xhr.responseText).scripts;"
    ...
    "  var dd=eb('meterDropdown');"
    "  dd.innerHTML='';"
    "  if(!filtered.length){"
    "    dd.innerHTML='<div style=\"padding:14px 12px;color:#999;font-style:italic;text-align:center\">No meters found</div>';"
    "  }else{"
    "    filtered.forEach(function(m){"
    "      var d=document.createElement('div');"
    "      d.className='meter-item';"
    "      d.textContent=m.name;"
    "      d.onclick=function(e){"
    "        e.stopPropagation();"
    "        selectMeter(m.name,m.filename);"
    "      };"
    "      dd.appendChild(d);"
    "    });"
    "  }"
    ...
    "  setTimeout(function(){eb('meterDropdown').classList.remove('show');},200);"
    "}"
    "function selectMeter(name,filename){"
    "  selectedMeter={name:name,filename:filename};"
    "  eb('meterSearch').value=name;"
    "  eb('loadMeterScript').disabled=false;"
    "  hideDropdown();"
    "}"
    "function filterMeters(){"
    "  var val=eb('meterSearch').value.toUpperCase();"
    "  return val===''?meterData:meterData.filter(function(m){return m.name.toUpperCase().indexOf(val)>-1;});"
    "}"
    "function loadSelectedScript(){"
    "  if(!selectedMeter)return;"
    "  eb('charNum').innerHTML='Loading...';"
    "  var xhr=new XMLHttpRequest();"
    "  xhr.open('GET','https://raw.githubusercontent.com/iot-maker/tasmota-smart-meter-scripts/main/scripts/'+selectedMeter.filename,true);"
    "  xhr.onreadystatechange=function(){"
    "    if(xhr.readyState==4){"
    "      if(xhr.status==200){"
    "        eb('t1').value=xhr.responseText;"
    "        eb('charNum').innerHTML='Script loaded - Enable and Save!';"
    "      }else{"
    "        eb('charNum').innerHTML='Failed to load script';"
    "      }"
    "    }"
    "  };"
    "  xhr.send();"
    "}"
    "if(typeof eb!=='undefined'&&eb('meterSearch')){"
    "  window.addEventListener('load',loadSmartMeterList);"
    "  var inp=eb('meterSearch');"
    "  inp.addEventListener('input',function(){showDropdown(filterMeters());});"
    "  inp.addEventListener('focus',function(){if(meterData.length)showDropdown(filterMeters());});"
    "  inp.addEventListener('blur',hideDropdown);"
    "  document.addEventListener('click',function(e){if(!e.target.closest('.meter-autocomplete'))eb('meterDropdown').classList.remove('show');});"
    "}"
    "</script>";

const char HTTP_SMARTMETER_DROPDOWN[] PROGMEM =
    "<style>"
    ".meter-autocomplete{position:relative;width:100%%;max-width:400px}"
    ".meter-autocomplete input{width:100%%;padding:10px;font-size:16px;border:2px solid #ccc;border-radius:4px}"
    ".meter-autocomplete input:focus{outline:none;border-color:#2196F3}"
    "#meterDropdown{position:absolute;background:#fff;border:2px solid #2196F3;border-top:none;border-radius:0 0 4px 4px;max-height:300px;overflow-y:auto;width:100%%;z-index:1000;display:none;box-shadow:0 4px 8px rgba(0,0,0,0.2);margin-top:-2px}"
    "#meterDropdown.show{display:block}"
    ".meter-item{padding:12px 10px;cursor:pointer;font-size:16px;color:#333;border-bottom:1px solid #f0f0f0;-webkit-tap-highlight-color:transparent}"
    ".meter-item:hover,.meter-item.selected{background:#e7f3ff}"
    ".meter-item:active{background:#2196F3;color:#fff}"
    "</style>"
    "<fieldset style='margin-bottom:10px'><legend><b>Smart Meter Templates</b></legend>"
    "<label for='meterSearch'>Search and select meter:</label>"
    "<div class='meter-autocomplete'>"
    "<input type='text' id='meterSearch' placeholder='Type meter name or tap to browse...' autocomplete='off'>"
    "<div id='meterDropdown'></div>"
    "</div>"
    "<button type='button' id='loadMeterScript' onclick='loadSelectedScript()' disabled style='margin:10px 0 5px 0;padding:10px 20px;font-size:16px'>Load Script</button>"
    "</fieldset>";
...
void HandleScriptConfiguration(void) {
    ...
    WSContentSend_P(HTTP_SMARTMETER_DROPDOWN);
