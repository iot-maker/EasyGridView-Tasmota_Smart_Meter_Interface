...
const char HTTP_SCRIPT_SMARTMETER_JS[] PROGMEM =
    ...
    "          meterData=data.scripts;"
    "          populateDatalist();"
    "        }catch(e){"
    "          console.error('Error loading meter data:', e);"
    "        }"
    "      }else{"
    ...
    "  var datalist=eb('meterList');"
    "  datalist.innerHTML='';"
    "  meterData.forEach(function(meter){"
    "    datalist.innerHTML+='<option value=\"'+meter.name+'\" data-filename=\"'+meter.filename+'\">';"
    "  });"
    "}"
    "function enableLoadButton(){"
    "  var searchValue=eb('meterSearch').value;"
    "  var hasSelection=meterData.some(function(m){return m.name===searchValue;});"
    "  eb('loadMeterScript').disabled=!hasSelection;"
    "}"
    "function loadSelectedScript(){"
    "  var filename=getFilenameFromSearch();"
    "  if(!filename)return;"
    "  var xhr=new XMLHttpRequest();"
    "  xhr.open('GET','https://raw.githubusercontent.com/iot-maker/tasmota-smart-meter-scripts/main/scripts/'+filename,true);"
    "  xhr.onreadystatechange=function(){"
    "    if(xhr.readyState==4){"
    "      if(xhr.status==200){"
    ...
    "  xhr.send();"
    "}"
    "function getFilenameFromSearch(){"
    "  var searchValue=eb('meterSearch').value;"
    "  var meter=meterData.find(function(m){return m.name===searchValue;});"
    "  return meter?meter.filename:'';"
    "}"
...
const char HTTP_SMARTMETER_DROPDOWN[] PROGMEM =
    ...
    "<label for='meterSearch'>Search meter model:</label>"
    "<input list='meterList' id='meterSearch' placeholder='Search meters...' style='width:70%%;margin:5px' oninput='enableLoadButton()'>"
    "<datalist id='meterList'></datalist>"
    "<br><button type='button' id='loadMeterScript' onclick='loadSelectedScript()' disabled style='margin:5px'>Load Script</button>"
    "</fieldset>";
